...
public class EntityRegister {
    ...
    private Map<Client.Location, List<Client>> groupClients(List<Client> clients) {
        ...
        for (Client client : clients) {
            switch (client.getLocation()) {
                ...
                case UKNOWN -> unknownLocation.add(client);
            }
        }
        ...
        clientsByLocation.put(Client.Location.UKNOWN, unknownLocation);
    ...
    private void registerPets(Client client) {
        String message = "Do you want add more pets for the current client? (y/n):";
        do {
            addPet(client);
        } while (verifyRepeating(message));
